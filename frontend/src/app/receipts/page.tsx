...
export default async function ReceiptsPage() {
  ...
  return (
    <PageShell
      title="Receções"
      description="Receber parcial/total com lotes e validade."
    >
      <SectionGrid>
        <SectionCard title="Receber encomenda">
          <p style={{ margin: 0, color: '#555' }}>
            Selecione uma encomenda para registar a receção.
          </p>
        </SectionCard>
        <SectionCard title="Encomendas recentes">
          {ordersResult.error ? (
            <p style={{ margin: 0, color: '#b00020' }}>{ordersResult.error}</p>
          ) : orders.length === 0 ? (
            <p style={{ margin: 0, color: '#555' }}>Sem encomendas para mostrar.</p>
          ) : (
